...
module.exports = function(rootDirectory, logger, indexDocument, errorDocument) {
  ...
  function buildResponse(req, res, status, object) {
    ...
    if (req.method === "HEAD") {
      object && object.content && object.content.destroy();
      res.end();
    } else {
      if (!object.content) {
        object.content = new Readable();
        object.content.push('');
        object.content.push(null);
      }
  ...
  return {
    ...
    getBucket(req, res) {
      if (req.method === 'HEAD') {
          store.getBucket(req.params.bucket, (err, bucket) => {
            if (err) {
              const template = templateBuilder.buildError(
                "InternalError",
                "We encountered an internal error. Please try again."
            ...
            if (!bucket) {
              logger.error('No bucket found for "%s"', req.params.bucket);
              const template = templateBuilder.buildBucketNotFound(req.params.bucket);
              return buildXmlResponse(res, 404, template);
            }
            return res.status(200).end();
          });
