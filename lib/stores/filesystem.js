...
class FilesystemStore {
  ...
  async listObjects(bucket, options, done) {
    const bucketPath = this.getBucketPath(bucket);
    const commonPrefixes = new Set();

    const prefix = path.join(bucketPath, options.prefix || "");
    let folder = prefix;
    if (!prefix.endsWith("/")) {
      folder = path.dirname(prefix);
    }
    let keys = await utils.walk(folder, prefix);
    keys = keys.map(key => key.slice(bucketPath.length + 1));
